...
Il diagramma distribuzione cloud seguente illustra un esempio di lacune nella sicurezza nella **casella rossa** Il **casella di colore giallo** Mostra chat room per l'ottimizzazione delle Appliance virtuali di rete tra carichi di lavoro.
...
L'implementazione di un data center virtuale non rappresenta solo i carichi di lavoro delle applicazioni nel cloud, ma anche la rete, la sicurezza, la gestione e l'infrastruttura (ad esempio, DNS e servizi directory). Come aumentare dei carichi di lavoro dell'organizzazione. è possibile spostare in Azure, è importante considerare l'infrastruttura di supporto e questi carichi di lavoro vengono inseriti in oggetti. Un'attenta valutazione di come le risorse sono strutturate può evitare il proliferare di centinaia di "isole di carichi di lavoro" che devono essere gestite separatamente, ognuna con i propri flussi di dati, modelli di sicurezza e problematiche di conformità.
...
Gli spoke possono anche essere interconnessi a uno spoke che funge da hub. Questo approccio consente di creare una gerarchia a due livelli: lo spoke nel livello superiore (livello 0) diventa l'hub degli spoke inferiori (livello 1) della gerarchia. Gli spoke di un'implementazione del Data Center virtuale sono necessari per inoltrare il traffico all'hub centrale, in modo che può far passare il traffico verso la destinazione nella rete locale o la rete internet pubblica. Un'architettura a due livelli di hub introduce un routing complesso che elimina i vantaggi di una relazione hub-spoke semplice.
...
È consigliabile in generale che i diritti e i privilegi di accesso siano basati sui gruppi. Gestione di gruppi anziché utenti singoli semplifica la manutenzione dei criteri di accesso, fornendo un modo coerente per la gestione tra i team e consente di ridurre al minimo gli errori di configurazione. L'assegnazione e la rimozione di utenti nei gruppi appropriati consente di mantenere aggiornati i privilegi di un utente specifico.
...
Il Data Center virtuale è progettato in modo che i gruppi creati per il gruppo IT centrale, gestiscono l'hub abbiano gruppi corrispondenti a livello di carico di lavoro. Oltre a gestire solo le risorse dell'hub, i gruppi IT centrali possono controllare l'accesso esterno e le autorizzazioni di primo livello per la sottoscrizione. I gruppi del carico di lavoro sono anche in grado di controllare le risorse e le autorizzazioni della rete virtuale in modo indipendente da centrale IT.
...
Nell'ambito IT un ambiente (o livello) è in genere un sistema in cui vengono distribuite ed eseguite più applicazioni. Le grandi imprese usano un ambiente di sviluppo (in cui le modifiche vengono apportate e testate) e un ambiente di produzione (ciò che gli utenti finali usano). Tali ambienti sono separati, spesso con diversi ambienti di staging, per consentire la distribuzione a fasi (rollout), il test e il ripristino dello stato precedente in caso di problemi. Le architetture di distribuzione variano in modo significativo, ma in genere il processo di base, che prevede l'avvio con lo sviluppo (DEV) e la fine con la produzione (PROD), è ancora seguito.
...
[**Azure Load Balancer**][ALB] offre un servizio a disponibilità elevata di livello 4 (TCP, UDP), che consente di distribuire il traffico in ingresso tra le istanze del servizio definite in set con carico bilanciato. Il traffico inviato al servizio di bilanciamento del carico dagli endpoint front-end (endpoint IP pubblici o endpoint IP privati) può essere ridistribuito con o senza conversione degli indirizzi in un set di pool di indirizzi IP back-end (esempi sono macchine virtuali o Appliance virtuali di rete).

Azure Load Balancer può verificare l'integrità delle varie istanze del server nonché, e se un'istanza non riesce a rispondere a un probe, il bilanciamento del carico interrompe l'invio di traffico all'istanza del tipo non integro. Nel data center virtuale viene distribuito un servizio di bilanciamento del carico esterno per hub e spoke. Nell'hub, il servizio di bilanciamento del carico consente di instradare in modo efficiente il traffico ai servizi negli spoke e negli spoke i servizi di bilanciamento del carico vengono usati per gestire il traffico delle applicazioni.
...
- I [log eventi][NSGLog] forniscono informazioni su quali regole dei gruppi di sicurezza di rete sono applicate alle macchine virtuali e ai ruoli delle istanze in base all'indirizzo MAC.
...
In questo articolo si è finora parlato della progettazione di un singolo data center virtuale e sono stati descritti i componenti e l'architettura di base che contribuiscono alla resilienza. Le funzionalità di Azure, ad esempio Azure load balancer, Appliance virtuali di rete, set di disponibilità, set di scalabilità e altri meccanismi di contribuiscono a un sistema che consente di creare a tinta unita con livelli di contratto di servizio nei servizi di produzione.
...
- Peering reti virtuali - peering reti virtuali può connettere hub in regioni.
- Peering quando gli hub in ogni implementazione di Data Center virtuale sono connesse allo stesso circuito ExpressRoute privato di ExpressRoute.
- Più circuiti ExpressRoute connessi tramite il backbone aziendale e le implementazioni del Data Center virtuale più connessi ai circuiti di ExpressRoute.
- Connessioni VPN Site-to-Site tra la zona dell'hub delle implementazioni del Data Center virtuale in ogni area di Azure.
