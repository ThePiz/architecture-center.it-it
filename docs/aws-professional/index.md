...
- **Amministratore account**. Proprietario della sottoscrizione e account di fatturazione delle risorse usate nella sottoscrizione. L'amministratore account può essere modificato solo trasferendo la proprietà della sottoscrizione.
...
- **Coamministratore**. Possono essere presenti più account coamministratore assegnati a una sottoscrizione. I coamministratori non possono cambiare l'amministratore del servizio, ma hanno il controllo completo sulle risorse e sugli utenti della sottoscrizione.
...
Analogamente agli account AWS, le sottoscrizioni presentano limiti e quote di servizio predefiniti. Per informazioni dettagliate su tali limiti, vedere [Sottoscrizione di Azure e limiti, quote e vincoli dei servizi](/azure/azure-subscription-service-limits).
...
- [Come aggiungere o modificare i ruoli di amministratore di Azure](/azure/billing/billing-add-change-azure-subscription-administrator)

- [Come scaricare la fattura e i dati di utilizzo giornalieri di Azure](/azure/billing/billing-download-azure-invoice-daily-usage-date)
...
È possibile organizzare le risorse anche usando i [tag](/azure/azure-resource-manager/resource-group-using-tags), ovvero coppie chiave-valore che consentono di raggruppare le risorse della sottoscrizione, indipendentemente dalla loro appartenenza al gruppo di risorse.

### <a name="management-interfaces"></a>Interfacce di gestione

Azure consente di gestire le risorse in modi diversi:

- [Interfaccia Web](/azure/azure-resource-manager/resource-group-portal).
...
- [Riga di comando](/azure/azure-resource-manager/cli-azure-resource-manager).
...
- [Modelli](/azure/azure-resource-manager/resource-group-authoring-templates)
...
- [Linee guida sui gruppi di risorse di Azure](/azure/azure-resource-manager/resource-group-overview#resource-groups)
...
- [Aree e disponibilità per le macchine virtuali in Azure](/azure/virtual-machines/linux/regions-and-availability)
...
Non tutti i servizi e i prodotti Azure sono disponibili in tutte le regioni. Consultare la pagina [Prodotti in base all'area](https://azure.microsoft.com/global-infrastructure/services/) per altre informazioni. Per informazioni sui tempi di attività garantiti e le politiche di credito per i tempi di inattività, leggere la pagina dei [Contratti di servizio](https://azure.microsoft.com/support/legal/sla/).
...
- [Dimensioni delle macchine virtuali in Azure (Windows)](/azure/virtual-machines/windows/sizes)

- [Dimensioni delle macchine virtuali in Azure (Linux)](/azure/virtual-machines/linux/sizes)
...
L'archiviazione permanente dei dati delle macchine virtuali di Azure viene fornita dai [dischi dati](/azure/virtual-machines/linux/about-disks-and-vhds) che risiedono nell'archivio BLOB, analogamente all'archiviazione dei volumi dei dischi da parte delle istanze di EC2 su Elastic Block Store (EBS). Anche l'[archiviazione temporanea di Azure](https://blogs.msdn.microsoft.com/mast/2013/12/06/understanding-the-temporary-drive-on-windows-azure-virtual-machines/) offre alle macchine virtuali la stessa archiviazione temporanea in lettura/scrittura e a bassa latenza delle istanze Storage di EC2.

L'IO su disco con prestazioni superiori è supportato tramite l'utilizzo dell'[archiviazione Premium di Azure](/azure/virtual-machines/windows/premium-storage). Il metodo è analogo alle opzioni di archiviazione di Provisioned IOPS offerte da AWS.
...
[Funzioni di Azure](https://azure.microsoft.com/services/functions/) è il principale equivalente di Lambda AWS per quanto concerne l'offerta di codice senza server e su richiesta. Le funzionalità Lambda si sovrappongono tuttavia anche ad altri servizi di Azure:

- [Processi Web](/azure/app-service/web-sites-create-web-jobs) consente di creare attività in background pianificate o in esecuzione continua.

- [App per la logica](https://azure.microsoft.com/services/logic-apps/) fornisce servizi di gestione delle regole aziendali, comunicazioni e integrazione.
...
- I [set di scalabilità delle macchine virtuali](/azure/virtual-machine-scale-sets/overview) consentono di distribuire e gestire un set identico di macchine virtuali. È possibile ridimensionare il numero di istanze in base alle esigenze prestazionali.

- [Ridimensionamento automatico del Servizio app](/azure/app-service/web-sites-scale) consente il ridimensionamento automatico delle soluzioni del Servizio app di Azure.

#### <a name="container-service"></a>Servizio contenitore

Il [servizio Azure Kubernetes](/azure/aks/intro-kubernetes) supporta i contenitori Docker gestiti tramite Kubernetes.
...
- [Azure Batch](/azure/batch/batch-technical-overview) consente di gestire attività con elevato utilizzo di calcolo in una raccolta scalabile di macchine virtuali.

- [Service Fabric](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-overview) è piattaforma per lo sviluppo e l'hosting di soluzioni di [microservizi](/azure/service-fabric/service-fabric-overview-microservices) scalabili.

#### <a name="see-also"></a>Vedere anche 

- [Creare una macchina virtuale Linux in Azure usando il portale](/azure/virtual-machines/linux/quick-create-portal)

- [Architettura di riferimento di Azure: Eseguire una macchina virtuale Linux in Azure](/azure/architecture/reference-architectures/n-tier/linux-vm)

- [Introduzione alle app Web Node.js nel servizio app di Azure](/azure/app-service/app-service-web-get-started-nodejs)

- [Architettura di riferimento di Azure: Esecuzione di un'applicazione Web di base in Azure](/azure/architecture/reference-architectures/app-service-web-app/basic-web-app)

- [Creare la prima funzione di Azure](/azure/azure-functions/functions-create-first-azure-function)
...
- **Simple Storage Service (S3)**. Archiviazione oggetti di base che rende disponibili i dati tramite un'API accessibile da Internet.

- **Elastic Block Storage (EBS)**. Archiviazione a livello di blocco progettata per l'accesso da parte di una sola macchina virtuale.

- **Elastic File System (EFS)**. Archiviazione di file progettata come archiviazione condivisa per migliaia di istanze di EC2.

In Archiviazione di Azure, [gli account di archiviazione](/azure/storage/common/storage-quickstart-create-account) associati a una sottoscrizione consentono di creare e gestire i servizi di archiviazione seguenti:

- L'[Archiviazione BLOB](/azure/storage/common/storage-quickstart-create-account) può archiviare qualsiasi tipo di dati di testo o binari, ad esempio un documento, un file multimediale o un programma di installazione di un'applicazione. Può essere impostata per l'accesso privato o per condividere contenuto pubblico su Internet. Archiviazione BLOB ha le stesse finalità di AWS S3 ed EBS.
- L'[Archiviazione tabelle](/azure/cosmos-db/table-storage-how-to-use-nodejs) archivia set di dati strutturati. L'archiviazione tabelle usa un archivio dati chiave-attributo NoSQL, che consente lo sviluppo e l'accesso rapido a grandi quantità di dati. È simile ai servizi SimpleDB e DynamoDB di AWS.

- L'[Archiviazione code](/azure/storage/queues/storage-nodejs-how-to-use-queues) offre un sistema di messaggistica per l'elaborazione dei flussi di lavoro e per la comunicazione tra componenti dei servizi cloud.

- L'[Archiviazione file](/azure/storage/files/storage-java-how-to-use-file-storage) offre un'archiviazione condivisa per le applicazioni che usano il protocollo standard Server Message Block (SMB). L'archiviazione file viene usata in modo analogo a EFS nella piattaforma AWS.
...
- [Elenco di controllo di prestazioni e scalabilità per Archiviazione di Microsoft Azure](/azure/storage/common/storage-performance-checklist)

- [Guida alla sicurezza di Archiviazione di Azure](/azure/storage/common/storage-security-guide)

- [Procedure consigliate per l'uso di reti per la distribuzione di contenuti (CDN)](/azure/architecture/best-practices/cdn)
...
- Il servizio [Gestione traffico][traffic-manager] offre funzionalità di routing del traffico a livello di DNS, di bilanciamento del carico e di failover.
...
- [Azure Active Directory](https://azure.microsoft.com/documentation/articles/active-directory-whatis/) è servizio per la gestione delle identità e delle directory basato sul cloud.
...
- [Azure Active Directory Domain Services](https://azure.microsoft.com/documentation/articles/active-directory-ds-overview/) è un servizio di hosting del controller di dominio, che consente l'aggiunta a un dominio compatibile con Active Directory e funzionalità di gestione utente.

#### <a name="web-application-firewall"></a>Web application firewall

Oltre al [Web application firewall del Gateway applicazione](/azure/application-gateway/waf-overview), è anche possibile usare Web application firewall di fornitori di terze parti come [Barracuda Networks](https://azure.microsoft.com/marketplace/partners/barracudanetworks/waf/).

#### <a name="see-also"></a>Vedere anche 

- [Introduzione alla sicurezza in Microsoft Azure](/azure/security)

- [Procedure consigliate per la sicurezza con il controllo di accesso e la gestione delle identità di Azure](/azure/security/azure-security-identity-management-best-practices)
...
- [Archiviazione code](/azure/storage/queues/storage-nodejs-how-to-use-queues) è un servizio di messaggistica cloud che consente la comunicazione tra i componenti dell'applicazione nella piattaforma Azure.

- [Bus di servizio](https://azure.microsoft.com/services/service-bus/) è un sistema di messaggistica più affidabile per la connessione di applicazioni, servizi e dispositivi. Tramite l'utilizzo dell'[inoltro del bus di servizio](/azure/service-bus-relay/relay-what-is-it) correlato, Bus di servizio consente anche la connessione a servizi e applicazioni ospitati in modalità remota.
...
- [Come usare l'archiviazione di accodamento da Node.js](/azure/storage/queues/storage-nodejs-how-to-use-queues)

- [Come usare le code del bus di servizio](/azure/service-bus-messaging/service-bus-nodejs-how-to-use-queues)
...
- [SQL Data Warehouse](https://azure.microsoft.com/documentation/services/sql-data-warehouse/) è archivio dati relazionale su larga scala.

- [Data Lake Store](https://azure.microsoft.com/documentation/services/data-lake-store/) è un archivio su larga scala ottimizzato per carichi di lavoro di analisi di Big Data.

- [Machine Learning](https://azure.microsoft.com/documentation/services/machine-learning/) consente di creare e applicare analisi predittive sui dati.

- [Analisi di flusso](https://azure.microsoft.com/documentation/services/stream-analytics/) offre analisi dei dati in tempo reale.

- [Data Lake Analytics](https://azure.microsoft.com/documentation/articles/data-lake-analytics-overview/) è un servizio di analisi su larga scala ottimizzato per l'utilizzo con Data Lake Store

- [Power BI](https://powerbi.microsoft.com/) è un servizio usato per migliorare la visualizzazione dei dati.
...
- [Blog di Azure Data Lake e Azure HDInsight](https://blogs.msdn.microsoft.com/azuredatalake/)
...
- [Indicazioni per il monitoraggio e la diagnostica](https://azure.microsoft.com/documentation/articles/best-practices-monitoring/)
