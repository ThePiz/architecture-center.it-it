...
Questa architettura di riferimento mostra un set di procedure comprovate per l'esecuzione di più macchine virtuali Linux in un set di scalabilità con un bilanciamento del carico per migliorare la disponibilità e la scalabilità. L'architettura può essere usata per qualsiasi carico di lavoro senza stato, ad esempio un server Web, e rappresenta una base per la distribuzione di applicazioni a più livelli. [**Distribuire questa soluzione**.](#deploy-the-solution)
