...
ms.locfileid: "51527729"
---
# <a name="basic-web-application-in-azure"></a>Applicazione Web di base in Azure

Questa architettura di riferimento mostra un set di procedure collaudate per un'applicazione Web che usa [Servizio app di Azure][app-service] e [Database SQL di Azure][sql-db]. [**Distribuire questa soluzione.**](#deploy-the-solution)
...
Se è necessario un livello di servizio o di prestazioni superiore per il database SQL, è possibile scalare verticalmente i singoli database senza tempi di inattività dell'applicazione. Per altre informazioni, vedere l'articolo relativo alle [opzioni e alle prestazioni del database SQL disponibili in ogni livello di servizio][sql-db-scale].
