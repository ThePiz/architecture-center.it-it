...
Questo articolo illustra una serie di procedure consolidate per l'esecuzione di una macchina virtuale Linux in Azure. Include suggerimenti per il provisioning della macchina virtuale insieme ai componenti di rete e archiviazione. [**Distribuire questa soluzione**](#deploy-the-solution).
...
- **Rete virtuale**. Ogni macchina virtuale di Azure viene distribuita in una rete virtuale che può essere suddivisa in più subnet.
...
1. Creare una coppia di chiavi SSH. Per altre informazioni, vedere [Come creare e usare una coppia di chiavi SSH pubblica e privata per le macchine virtuali Linux in Azure](/azure/virtual-machines/linux/mac-create-ssh-keys).

2. Passare alla cartella `virtual-machines/single-vm/parameters/linux` per il repository scaricato nel passaggio dei prerequisiti precedente.

3. Aprire il file `single-vm-v2.json` e immettere un nome utente e una chiave SSH pubblica tra virgolette, quindi salvare il file.
...
4. Eseguire `azbb` per distribuire la macchina virtuale di esempio, come illustrato di seguito.
