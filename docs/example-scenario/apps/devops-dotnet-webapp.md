...
Questo scenario include una pipeline DevOps per un'applicazione Web .NET che usa Visual Studio Team Services (VSTS). Il flusso dei dati nello scenario avviene come segue:
...
* I [gruppi di risorse][resource-groups] sono un contenitore logico per le risorse di Azure e offrono anche un limite per il controllo di accesso per il piano di gestione. Un gruppo di risorse può essere considerato come una "unità di distribuzione".
* [Visual Studio Team Services (VSTS)][vsts] è un servizio che consente di gestire il ciclo di vita dello sviluppo end-to-end, dalla pianificazione e dalla gestione dei progetti e del codice fino alla compilazione e al rilascio.
...
La scopo dell'**[integrazione continua][continuous-integration]** è garantire una compilazione stabile, con il commit di modifiche frequenti a una base di codice condivisa da parte di più sviluppatori o team.
Nell'ambito della pipeline di integrazione continua è consigliabile:

* Archiviare spesso quantità ridotte di codice, evitando l'invio in batch di modifiche più estese o complesse perché può essere più difficile eseguirne correttamente il merge.
* Eseguire unit test sui componenti dell'applicazione con code coverage sufficiente, includendo i percorsi non corretti.
...
Lo scopo della **[distribuzione continua][continuous-delivery]** è garantire non solo una compilazione stabile, ma anche una distribuzione stabile. Realizzare la distribuzione continua risulta così un po' più difficile, perché sono necessari una configurazione specifica dell'ambiente e un meccanismo per impostare correttamente tali valori.

È inoltre necessario un coverage dei test di integrazione sufficiente a garantire che i vari componenti siano configurati e funzionino correttamente end-to-end.

Potrebbe anche essere necessario configurare e reimpostare dati specifici dell'ambiente e gestire versioni dello schema del database.

Il recapito continuo può anche estendersi ad ambienti di test di carico e test di accettazione utente.
...
* Avviare il recapito continuo il prima possibile nel ciclo di vita del progetto. Più viene rimandato, più difficile diventa.
* A test di integrazione e unit test dovrà essere assegnata la stessa priorità delle funzionalità del progetto.
...
* Proteggere le informazioni di configurazione riservate negli strumenti di gestione del rilascio oppure ricorrendo a un modulo di protezione hardware o a [Key Vault][azure-key-vault] durante il processo di rilascio. Non archiviare informazioni di configurazione riservate nel controllo del codice sorgente.

**Apprendimento continuo**. Il monitoraggio più efficace di un ambiente di distribuzione continua è offerto dagli strumenti di monitoraggio delle prestazioni applicative (APM, Application Performance Monitoring), come ad esempio Microsoft [Application Insights][application-insights]. Un monitoraggio sufficientemente approfondito del carico di lavoro di un'applicazione è essenziale per conoscere i bug e le prestazioni in condizioni di carico. [Application Insights può essere integrato in VSTS per supportare il monitoraggio continuo della pipeline di distribuzione continua][app-insights-cd-monitoring]. In questo modo è possibile abilitare l'avanzamento automatico alla fase successiva senza intervento umano oppure il rollback se viene rilevato un avviso.
...
* È necessario un account Visual Studio Team Services (VSTS) esistente. Per altri dettagli, vedere la procedura per la [creazione di un account Visual Studio Team Services (VSTS)][vsts-account-create].
...
* Valutare la possibilità di usare [Application Insights][application-insights] e altri strumenti di monitoraggio il prima possibile nella pipeline di versione. La maggior parte delle organizzazioni inizia il monitoraggio solo nell'ambiente di produzione, nonostante sia possibile identificare i potenziali bug in fasi precedenti del processo ed evitarne l'impatto sugli utenti in produzione.

## <a name="pricing"></a>Prezzi

I costi di Visual Studio Team Services dipenderanno dal numero di utenti dell'organizzazione che devono accedere, nonché da fattori come il numero di processi di compilazione/rilascio simultanei necessari e il numero di utenti di test. Per informazioni più dettagliate, vedere la pagina [Prezzi di Visual Studio Team Services][vsts-pricing-page].

* [Visual Studio Team Services (VSTS)][vsts-pricing-calculator] è un servizio che consente di gestire il ciclo di vita dello sviluppo con pagamento per utente su base mensile. Addebiti aggiuntivi possono essere previsti a seconda delle pipeline simultanee necessarie e di eventuali licenze utente Basic o utenti di test aggiuntivi.

## <a name="related-resources"></a>Risorse correlate

* [Informazioni su DevOps][devops-whatis]
* [DevOps in Microsoft: come viene usato Visual Studio Team Services][devops-microsoft]
* [Esercitazioni dettagliate: DevOps con Visual Studio Team Services][devops-with-vsts]
