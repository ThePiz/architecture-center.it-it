...
Questo scenario illustra come distribuire e gestire un server Jenkins scalabile, di livello aziendale in Azure la protezione con single sign-on (SSO). L'architettura usa anche Monitoraggio di Azure per monitorare lo stato del server Jenkins. [**Distribuire questa soluzione**](#deploy-the-solution).
...
- **Server Jenkins**. Distribuzione di una macchina virtuale per eseguire [Jenkins] [ azure-market] come un server di automazione e funge da Master Jenkins. Per distribuire questa macchina virtuale, usare il [modello di soluzione per Jenkins in Azure][solution]. In Azure Marketplace sono disponibili altre offerte Jenkins.
