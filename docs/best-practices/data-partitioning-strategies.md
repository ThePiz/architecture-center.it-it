...
Il diagramma seguente illustra la struttura logica di un account di archiviazione di esempio. Gli account di archiviazione contengono tre tabelle: Informazioni sul cliente, Informazioni sul prodotto e Informazioni relative all'ordine. 
...
* Se si eseguono regolarmente query che cercano i dati usando campi diversi dalle chiavi di partizione e di riga, valutare la possibilità di implementare un [Index Table Pattern] oppure di usare un diverso archivio dati che supporti l'indicizzazione, come Cosmos DB.
